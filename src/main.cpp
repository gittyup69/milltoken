...
#if defined(NDEBUG)
# error "MillToken cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3f9f0072d4a619df762af12f17d5c9196e4bccd78241c79e5c2dbb7dd36e1bef");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MillToken: starting difficulty is 1 / 2^12
...
int64 CTransaction::nMinTxFee = 900000;
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 900000;
...
const string strMessageMagic = "MillToken Signed Message:\n";
...
{
    // MillToken: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 21024000 blocks, which will occur approximately every 2 years
    nSubsidy >>= (nHeight / 21024000); // MillToken: 21.024m blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // MillToken: 1 day
static const int64 nTargetSpacing = .05 * 60; // MillToken: 4.2 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MillToken: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MillToken: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xae05edd017ff90d57fa97d7347c1ef71bafa370f09d89bd2ce62291a6ce3a481");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "11/30/2020 MillToken.us was purchased On the fourth day after Thanksgiving";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c6bd3712d48e065f34db5bd20d708d7762ee16a65b2fe0a13c1b309464c5a44ef1ea345b3da1e9525c7798e073c3265967bba5bacc2cab5824536f3354c6bb22") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1606729804;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086991972;

        if (fTestNet)
        {
            block.nTime    = 1606729799;
            block.nNonce   = 386133541;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x697e05f52827a7ff7d92f1384ae030b2d4907ddc45ab020e831242f028d1f0af"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xda, 0xb7, 0xe2, 0xad }; // MillToken: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MillTokenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MillTokenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MillTokenMiner(CWallet *pwallet)
{
    printf("MillTokenMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("milltoken-miner");
    ...
    try { loop {
        ...
        printf("Running MillTokenMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MillTokenMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MillTokenMiner, pwallet));
}
